...
interface PvPDuelsProps {
  lang?: 'en' | 'fr';
}

export function PvPDuels({ lang = 'en' }: PvPDuelsProps) {
      ...
      <div className="glass-panel rounded-3xl p-6 flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold text-slate-100 flex items-center gap-2">
            <Swords className="text-rose-400 w-6 h-6" />
            {lang === 'en' ? '60s PvP Duels Arena' : 'Arène PvP 60s'}
          </h2>
          <p className="text-slate-400 text-sm mt-1">
            {lang === 'en' 
              ? 'Challenge peer traders in high-speed 60-second crypto micro-prediction duels. Settled via Somnia Escrow.' 
        ...
        <div className="flex gap-2 bg-surface/60 p-1 rounded-xl border border-surfaceBorder">
          ...
          >
            {lang === 'en' ? 'Create Duel' : 'Créer un Duel'}
          </button>
        </div>
      </div>
          ...
          {duels.map(duel => (
            <div key={duel.id} className="glass-panel rounded-3xl overflow-hidden border border-surfaceBorder/60 hover:border-rose-500/30 transition-colors">
              <div className={`p-3 text-center text-xs font-bold uppercase tracking-wider ${
                ...
                {duel.status === 'WAITING' 
                  ? (lang === 'en' ? 'Waiting for Opponent' : 'En attente d\'adversaire')
                  : duel.status === 'LIVE' 
                  ? (lang === 'en' ? 'Live Duel in Progress' : 'Duel en cours') 
                  : (lang === 'en' ? 'Resolved' : 'Terminé')}
              </div>
              ...
              <div className="p-5">
                <div className="flex justify-between items-center mb-4">
                  ...
                  <div className="text-right">
                    <div className="text-[10px] text-slate-500 uppercase">{lang === 'en' ? 'Target Strike' : 'Prix Cible'}</div>
                ...
                <div className="flex justify-between items-center p-3 rounded-2xl bg-surface/50 border border-surfaceBorder mb-5">
                  <div>
                    <div className="text-[10px] text-slate-500">{lang === 'en' ? 'Total Pot' : 'Mise Totale (Pool)'}</div>
                    <div className="text-lg font-black text-emerald-400">${duel.stake * 2}</div>
                  </div>
                  <div className="text-right">
                    <div className="text-[10px] text-slate-500">{lang === 'en' ? 'Time Remaining' : 'Temps restant'}</div>
                ...
                {duel.status === 'WAITING' && (
                  <button className="w-full py-3 rounded-xl font-bold text-sm bg-rose-500/20 text-rose-400 hover:bg-rose-500/30 transition-all border border-rose-500/30 hover:border-rose-400">
                    {lang === 'en' ? `Join Duel ($${duel.stake} USDso)` : `Rejoindre (Mise: ${duel.stake}$)`}
                  </button>
                )}
                {duel.status === 'LIVE' && (
                  <button disabled className="w-full py-3 rounded-xl font-bold text-sm bg-surface/50 text-slate-500 border border-surfaceBorder cursor-not-allowed">
                    {lang === 'en' ? 'Duel Locked' : 'Duel Verrouillé'}
                  </button>
                )}
                {duel.status === 'RESOLVED' && (
                  <div className="w-full py-3 rounded-xl font-bold text-sm bg-slate-800/50 text-slate-400 border border-slate-700/50 text-center">
                    {lang === 'en' ? 'Winner: ' : 'Gagnant: '}<span className="font-mono text-emerald-400">{duel.winner}</span>
                  </div>
                )}
              </div>
            </div>
      ...
      {activeTab === 'create' && (
        <div className="glass-panel rounded-3xl p-8 max-w-lg mx-auto">
          <h3 className="text-lg font-bold text-slate-100 mb-6 flex items-center gap-2">
            ...
            {lang === 'en' ? 'Configure New Prediction Duel' : 'Configurer un nouveau Duel'}
          </h3>

          <div className="space-y-5">
            <div>
              <label className="block text-xs font-bold text-slate-400 mb-2">{lang === 'en' ? 'Asset (Symbol)' : 'Actif (Symbol)'}</label>
            ...
            <div>
              <label className="block text-xs font-bold text-slate-400 mb-2">{lang === 'en' ? 'Target Strike' : 'Prix Cible (Strike)'}</label>
            ...
            <div>
              <label className="block text-xs font-bold text-slate-400 mb-2">{lang === 'en' ? 'Your Prediction (60s Expiry)' : 'Votre Prédiction (à 60s)'}</label>
              <div className="grid grid-cols-2 gap-3">
                <button className="py-3 rounded-xl font-bold text-sm bg-emerald-500/20 text-emerald-400 border border-emerald-500/50">
                  {lang === 'en' ? 'Price >= Strike (YES)' : 'Prix >= Strike (YES)'}
                </button>
                <button className="py-3 rounded-xl font-bold text-sm bg-surface/50 text-slate-400 border border-surfaceBorder hover:border-slate-600">
                  {lang === 'en' ? 'Price < Strike (NO)' : 'Prix < Strike (NO)'}
                </button>
              </div>
            </div>

            <div>
              <label className="block text-xs font-bold text-slate-400 mb-2">{lang === 'en' ? 'Your Wager (USDso)' : 'Votre Mise (USDso)'}</label>
            ...
            <button className="w-full py-4 mt-4 rounded-xl font-black text-sm bg-gradient-to-r from-rose-600 to-orange-600 text-white shadow-xl shadow-rose-900/40 hover:from-rose-500 hover:to-orange-500 transition-all flex items-center justify-center gap-2">
              <Shield className="w-4 h-4" /> {lang === 'en' ? 'Deploy Escrow Duel' : 'Créer le Duel Escrow'}
            </button>
            <p className="text-center text-[10px] text-slate-500">
              {lang === 'en' ? 'Funds are securely locked in PvPDuelEscrow.sol on Somnia L1' : 'Les fonds seront verrouillés dans le smart contract PvPDuelEscrow.sol'}
            </p>
          </div>
        </div>
      )}
